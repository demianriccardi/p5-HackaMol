...
use PhysVecRole; # v0.001;#To test for version availability
...
my  $class = MooseX::ClassCompositor->new( 
                                          {class_basename => 'Test',}
                                         )->class_for('PhysVecRole');
map has_attribute_ok( $class, $_ ), @attributes;
map can_ok( $class, $_ ), @methods;

lives_ok{ $obj1 = $class->new(name => 'somephysvec', t => 1)
...
cmp_ok($obj1->get_charges(4), '==', 1.0, '5th _tcharges set to 1.0 and get_charges as expected');


lives_ok{ $obj2 = $class->new(name => 'someotherphysvec', t => 0)
        }                                     'Test creation of an obj2';

$obj1->set_coords($obj1->t,[1,0,0]);
$obj2->set_coords($obj2->t,[0,1,0]);
warning_is {$obj1->distance($obj2)} "you are comparing objects with different times", 
"carp warning about obj1 and obj2 distance with different t";

$obj1->t(0);
$obj1->set_coords($obj1->t, [1,0,0]);

my $sqrt_2 = sprintf("%.16f",1.4142135623730951); # double precision from fortran 90
...
cmp_ok(sprintf("%.16f", $obj1->distance($obj2)),
               '==',
       sprintf("%.16f", 2.0), "new coderef obj1 distance**2 between 100 and 010 to 16 decimal places: 2.0000000000000000");

cmp_ok(sprintf("%.16f", $obj2->distance($obj1)),
               '==',
              $sqrt_2, "default obj2 distance between 100 and 010 to 16 decimal places: $sqrt_2");
...
cmp_ok(sprintf("%.16f", $obj1->distance($obj2)), 
             '==', 
            $sqrt_2, "swap VectorReal into obj1->distance_coderef between 100 and 010 to 16 decimal places: $sqrt_2");

my $dq = $obj1->get_charges(1) - $obj1->get_charges(0);

cmp_ok($obj1->delta_charges(0,1), '==', $dq, "change in charges between t = 0 and t = 1: $dq" );
...
lives_ok{ $obj3 = $class->new(name => 'somephysvec3', t => 0, 
                              charges=>[-1,2], coords=>[[1,2,3],[2,3,4]]);
        }                                     'Test creation of an obj3';


is_deeply($obj3->delta_coords(0,1), [1,1,1], 'delta_coords' );
is_deeply($obj3->delta_charges(0,1), 3, 'delta_charges' );
