...
foreach my $t ( 1 .. 3 ) {
    ...
    $t3 = time;
    printf(
"$t time to push_coords( [2.000,2.000,2.000] ) for $natoms atoms: %10.3f\n",
        $t3 - $t2 );
    $t2 = time;
}
...
foreach my $t ( 1 .. 3 ) {
    ...
    $t3 = time;
    printf(
"Time to push_coords( [2.000,2.000,2.000] ) 100 times for atom $t: %.3g\n",
        $t3 - $t2 );
    $t2 = time;
}
...
my $t4 = time;
printf(
    "time to set_coords(0, [1.000 ,1.000,1.000]) for $natoms atoms: %10.3f\n",
    $t4 - $t3 );
